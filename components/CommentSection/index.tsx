...
function CommentSection({ blog_id }) {
  ...
  const getReplies = (commentID) => {
    return commentList
      ?.filter((comment) => comment.fields.parent?.sys.id == commentID)
      .sort((a, b) => new Date(a.sys.createdAt).getTime() - new Date(b.sys.createdAt).getTime());
  };
  useEffect(() => {
    getComment();
  }, []);
  const rootComments = commentList
    ?.filter((comment) => comment.fields.parent == null)
  ...
  return (
    <div className={styles.comments}>
      ...
      {rootComments?.length > 0 && <h4 className="comments-title">Bình luận</h4>}
          ...
          rootComments?.map((comment) => {
            return (
              <Comment
                comment={comment}
                key={comment.sys.id}
                replies={getReplies(comment.sys.id)}
